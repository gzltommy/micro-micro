...
import (
	...
	"github.com/gzltommy/category/domain/model"
	"github.com/gzltommy/category/domain/service"
	"github.com/gzltommy/category/proto/category"
	"context"
)
...
func (c *Category) CreateCategory(ctx context.Context, request *category.CategoryRequest, response *category.CreateCategoryResponse) error {
	ctg := &model.Category{}
	//赋值
	err := common.SwapTo(request, ctg)
	if err != nil {
		return err
	}
	categoryId, err := c.CategoryDataService.AddCategory(ctg)
	if err != nil {
		return err
	}
...
func (c *Category) UpdateCategory(ctx context.Context, request *category.CategoryRequest, response *category.UpdateCategoryResponse) error {
	...
	err = c.CategoryDataService.UpdateCategory(ctg)
	if err != nil {
		return err
	}
...
func (c *Category) FindCategoryByName(ctx context.Context, request *category.FindByNameRequest, response *category.CategoryResponse) error {
	ctg, err := c.CategoryDataService.FindCategoryByName(request.CategoryName)
	if err != nil {
		return err
	}
	return common.SwapTo(ctg, response)
...
func (c *Category) FindCategoryByID(ctx context.Context, request *category.FindByIdRequest, response *category.CategoryResponse) error {
	ctg, err := c.CategoryDataService.FindCategoryByID(request.CategoryId)
	if err != nil {
		return err
	}
	return common.SwapTo(ctg, response)
...
func (c *Category) FindCategoryByLevel(ctx context.Context, request *category.FindByLevelRequest, response *category.FindAllResponse) error {
	...
	return categoryToResponse(categorySlice, response)
}
...
func categoryToResponse(categorySlice []model.Category, response *category.FindAllResponse) error {
	for _, ctg := range categorySlice {
		cr := &category.CategoryResponse{}
		err := common.SwapTo(ctg, cr)
		if err != nil {
			return err
		}
