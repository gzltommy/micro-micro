...
import (
	"github.com/gzltommy/product/domain/model"
	"gorm.io/gorm"
)
...
func (u *ProductRepository) InitTable() error {
	return u.mysqlDb.AutoMigrate(&model.Product{}, &model.ProductSeo{}, &model.ProductImage{}, &model.ProductSize{})
}
...
//根据ID删除Product信息
// todo:除了下面通过事务的方式删除外，还可以通过在创建表示的时候指定外键约束规则来实现
