...
proto:
	...
	@docker run --rm -v $(PWD):$(PWD) -w $(PWD) gzltommy/protoc --proto_path=. --micro_out=paths=source_relative:. --go_out=paths=source_relative:. proto/product/*.proto
...
.PHONY: buildtx
buildtx: proto
	go build -o $(APPNAME)_client ./test/*.go
